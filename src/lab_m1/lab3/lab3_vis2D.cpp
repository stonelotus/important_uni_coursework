...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_Z)) {
        GLfloat zoomModifier = deltaTime * 3.f;

        logicSpace.x += logicSpace.width;
        logicSpace.y += logicSpace.height;
         
        logicSpace.width += zoomModifier;
        logicSpace.height += zoomModifier;
