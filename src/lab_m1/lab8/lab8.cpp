...
void Lab8::Update(float deltaTimeSeconds)
{
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.2, 1, 0.8));
    }
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.2, 1, 0.8));
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    glUniform3f(object_color, 0.4, 0.4, color.b);
    ...
    glUniform1i(type, spotlight);

    GLint cut_off_direction = glGetUniformLocation(shader->program, "cut_off_direction");
    glUniform1f(cut_off_direction, cut_off);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        {
            cut_off += deltaTime * 20.f;
            cut_off = cut_off > 360.f ? 360.f : cut_off;
        }
        if (window->KeyHold(GLFW_KEY_2))
        {
            cut_off -= deltaTime * 20.f;
            cut_off = cut_off < 0.f ? 0.f : cut_off;
        }
